...
export function PageCanvas({ page, assets, thumbnail = false }: Props) {
  ...
  const [customHighlightOpen, setCustomHighlightOpen] = useState(false);
  const [customHighlightHex, setCustomHighlightHex] = useState("#F6C945");
  ...
  function applyCustomHighlight() {
    const normalized = customHighlightHex.trim().replace(/^#/, "");
    if (!/^[0-9a-f]{6}$/i.test(normalized) || !textSelection) return;
    addTextHighlights(textSelection.rects, `#${normalized}`);
    setCustomHighlightOpen(false);
  }
  ...
  return (
    ...
    >
      ...
      {textSelection && <div role="toolbar" aria-label="高亮颜色" onPointerDown={(event) => { if (!(event.target instanceof HTMLInputElement)) event.preventDefault(); }} className="absolute z-30 flex h-10 items-center gap-1 rounded-xl border border-[#e5e9f0] bg-white px-2 shadow-[0_8px_24px_rgba(23,34,59,.18)]" style={{ left: textSelection.left, top: textSelection.top }}><span className="mr-1 text-xs font-semibold text-[#344054]">高亮</span>{highlightColors.map((color) => <button key={color.value} type="button" aria-label={`${color.name}高亮`} onClick={() => addTextHighlights(textSelection.rects, color.value)} className="h-6 w-6 rounded-full border-2 border-white shadow-[0_0_0_1px_rgba(23,34,59,.14)] transition-transform duration-150 hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315ee7] active:scale-95" style={{ backgroundColor: color.value }} />)}<button type="button" aria-label="自定义高亮颜色" aria-expanded={customHighlightOpen} onClick={() => { setCustomHighlightHex("#F6C945"); setCustomHighlightOpen((open) => !open); }} className={`ml-0.5 grid h-6 w-6 place-items-center rounded-full border border-dashed text-sm leading-none transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315ee7] ${customHighlightOpen ? "border-[#315ee7] bg-[#eef2ff] text-[#315ee7]" : "border-[#aeb6c4] bg-white text-[#667085] hover:border-[#315ee7] hover:text-[#315ee7]"}`}>+</button>{customHighlightOpen && <div role="dialog" aria-label="自定义高亮颜色面板" onPointerDown={(event) => event.stopPropagation()} className="absolute right-0 top-full z-50 mt-2 w-56 rounded-2xl border border-[#dce4f8] bg-white p-3 shadow-[0_14px_30px_rgba(23,34,59,.2)]"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-[#344054]">自定义高亮</span><button type="button" onClick={() => setCustomHighlightOpen(false)} className="rounded-md px-1.5 py-1 text-xs text-[#667085] hover:bg-[#f2f4f7]">关闭</button></div><div className="mt-3 flex items-center gap-3 rounded-xl bg-[#f7f8fb] p-2.5"><span aria-hidden className="h-10 w-10 shrink-0 rounded-lg border border-black/10 shadow-inner" style={{ backgroundColor: /^#[0-9a-f]{6}$/i.test(customHighlightHex) ? customHighlightHex : "#f6c945" }} /><label className="min-w-0 flex-1 text-[11px] font-medium text-[#667085]">Hex 色值<input aria-label="自定义高亮 Hex 色值" value={customHighlightHex} onChange={(event) => setCustomHighlightHex(event.target.value.toUpperCase())} maxLength={7} spellCheck={false} className="mt-1 block h-8 w-full rounded-lg border border-[#d8deea] bg-white px-2 text-xs font-semibold tracking-[.04em] text-[#17223b] outline-none focus:border-[#7b9af2] focus:ring-2 focus:ring-[#315ee7]/10" /></label></div><p className="mt-2 text-[11px] leading-4 text-[#8a93a5]">输入 6 位 Hex 色值，例如 #F6C945。</p><button type="button" disabled={!/^#[0-9a-f]{6}$/i.test(customHighlightHex)} onClick={applyCustomHighlight} className="mt-3 h-8 w-full rounded-lg bg-[#315ee7] text-xs font-semibold text-white transition hover:bg-[#2346bb] disabled:bg-[#b7c6f5]">应用此颜色</button></div>}</div>}
    </div>
  );
}
